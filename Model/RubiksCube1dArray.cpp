//
// Created by Abhinav Kumar on 19/09/24.
//
...
class RubiksCube1dArray : public RubiksCube {
    private:
        //Given a face index, row and col return it's falttened index
        ...
        void rotateFace(int ind) {
            ...
            for (int i=0; i<3; i++) cube[getIndex(ind, 0, i)] = temp_arr[getIndex(0, 2 - i, -i)];
            for (int i=0; i<3; i++) cube[getIndex(ind, i, 2)] = temp_arr[getIndex(0, 0, -i)];
            for (int i=0; i<3; i++) cube[getIndex(ind, 2, 2 - i)] = temp_arr[getIndex(0, 2, -i)];
